...
[source,bash]
----
kubectl -n appuio-cloud-reporting logs $(kubectl -n appuio-cloud-reporting get job -l cron-job-name=generate-invoices --field-selector status.successful=0 -o name | tail -n1)
----

. Remove any obstacles
+
Use the findings from the previous step to remove any obstacles that prevents the job from failing again.

. Rerun job
+
[source,bash]
----
name="generate-invoices-$($(command -v gdate || command -v date) +%s)"
