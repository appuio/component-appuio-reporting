...
APPUiO Reporting failed while trying to backfill Prometheus metering data to the reporting database.
...
echo $failed_timestamps \
  ...
  | xargs -n1 oc -n $NAMESPACE --as=system:admin debug job/$job_template -- appuio-reporting report --query-name=$query_name --begin
----
